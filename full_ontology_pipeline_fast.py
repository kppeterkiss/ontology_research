...
# =====================================================================
# LLM JELÖLT KIÉRTÉKELÉS (NIL) ÉS TISZTA DEFINÍCIÓ GENERÁLÁS
# =====================================================================
...
# PERFORMANCE LOGGING
def log_performance(source_doc, mention, decision, matched_id, reasoning):
    with open(PERFORMANCE_LOG_FILE, "a", encoding="utf-8") as f:
        f.write(json.dumps({
            "doc": source_doc, "mention": mention, "decision": decision,
            "matched_id": matched_id, "reasoning": reasoning
        }, ensure_ascii=False) + "\n")
...
def run_optimized_pipeline():
    ...
    # Csoportosítás dokumentum és egyedi kifejezések szintjére
    print("Adatok csoportosítása dokumentum és egyedi kifejezések szintjére...")
    doc_mention_map = {}
    for p in all_paragraphs:
        doc_id = p["doc_id"]

        # Ha a cikk már teljesen fel lett dolgozva egy korábbi futás során, ki hagyjuk a feladatlistából
    ...
    if total_jobs == 0:
        print(
            "\n[Minden kész] Nincs új feldolgozandó cikk a mappában, vagy az összes korábbi cikk sikeresen befejeződött!")
        return

    print(f"Csoportosítás kész. Összesen {total_jobs} egyedi feladat vár elvégzésre ebben a menetben.")
    ...
    for doc_id, mentions in doc_mention_map.items():
        ...
        for mention, context in mentions.items():
            print(f"[{current_job_num}/{total_jobs}] Kifejezés ellenőrzése: '{mention}'")
            current_job_num += 1

            # LÉPÉS 1: Lexikális Kapuőr (Azonnali egyezés ellenőrzése)
            lexical_match = check_lexical_exact_match(mention, CURRENT_ONTOLOGY)
            if lexical_match:
                print(f"   [->] Lexikális egyezés megtalálva! ID: {lexical_match['id']}. LLM hívás kihagyva.")
                log_performance(doc_id, mention, "EXISTING_LEXICAL", lexical_match['id'], "Exact string match filter.")
                continue

            # LÉPÉS 2: Kombinált BERT szűrés
            top_candidates = get_top_candidates_combined(mention, context, CURRENT_ONTOLOGY, top_n=2)

            # LÉPÉS 3: Ollama NIL döntés
            llama_output = ask_llama_nil_prediction(mention, context, top_candidates)
            decision = llama_output.get("decision", "NEW_CONCEPT")
            matched_id = llama_output.get("matched_concept_id")
            reasoning = llama_output.get("reasoning", "")

            print(f"   -> LLM döntés: {decision} | Indok: {reasoning}")
            log_performance(doc_id, mention, decision, matched_id, reasoning)

            # LÉPÉS 4: Új koncepció rögzítése és azonnali mentése
            if decision == "NEW_CONCEPT":
                print(f"   [!] Új koncepció elfogadva. Specifikus definíció generálása...")
                ...
                # Lemezre mentés (Dinamikus ontológia-növekedés)
                with open(ONTOLOGY_BASE_FILE, "w", encoding="utf-8") as f:
                    json.dump(CURRENT_ONTOLOGY, f, indent=2, ensure_ascii=False)

        # !!! CHECKPOINT: Amikor a cikk VÉGÉRE értünk, elmentjük a haladást !!!
        processed_documents.add(doc_id)
        save_progress(processed_documents)
        print(f"=== {doc_id} SIKERESEN FELDOLGOZVA. Checkpoint elmentve. ===")

    print("\nFolyamat sikeresen és biztonságosan véget ért.")
